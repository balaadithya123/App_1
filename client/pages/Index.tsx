      ...
      <div className="mx-auto max-w-[1060px]">
        <header className="flex items-center justify-between py-5 sm:py-7">
          <div className="flex items-center gap-2.5">
            <span className="flex h-8 w-8 items-center justify-center rounded-[10px] bg-primary text-sm font-extrabold text-primary-foreground">
              L
            </span>
            <span className="text-[17px] font-extrabold tracking-[-0.03em] text-primary">
              LocalWorker
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {loggedIn && (
              <Link
                to="/profile"
                aria-label="Profile"
                className="hidden h-10 w-10 items-center justify-center rounded-full border border-border bg-card text-foreground shadow-sm transition-colors min-[360px]:flex"
              >
                <UserRound size={19} />
              </Link>
            )}
            {!loggedIn && (
              <Link
                to="/register"
                className="hidden rounded-full border border-border bg-card px-5 py-2 text-[12px] font-bold text-foreground shadow-sm min-[360px]:block"
              >
                Register
              </Link>
            )}
        ...
        <div className="relative overflow-visible rounded-[24px] border-[2px] border-primary pb-3">
          <section className="relative overflow-hidden rounded-[21px] bg-card px-5 pb-20 pt-7 shadow-[0_18px_55px_rgba(24,55,62,0.10)] sm:px-10 sm:pb-24 sm:pt-10">
            ...
            <div className="relative max-w-[650px]">
              ...
              <h1 className="max-w-[620px] text-[36px] font-extrabold leading-[1.04] tracking-[-0.055em] text-foreground sm:text-5xl">
                Find local workers near you
              </h1>
              <p className="mt-3 max-w-[390px] text-[15px] leading-6 text-muted-foreground">
          ...
          <form
            ...
            className="relative z-10 mt-3 rounded-[18px] bg-card p-2 shadow-[0_14px_45px_rgba(24,55,62,0.16)] sm:flex sm:items-center sm:gap-2 sm:p-2.5"
          >
            <div className="flex min-w-0 flex-1 items-center rounded-[12px] border border-border bg-background px-3.5">
              ...
              <input
                ...
                className="h-11 min-w-0 w-full bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground"
              />
            </div>
            ...
            <button
              type="submit"
              className="mt-2 flex h-11 w-full items-center justify-center rounded-[12px] bg-primary px-7 text-sm font-bold text-primary-foreground sm:mt-0 sm:w-auto"
            >
              Search
            </button>
          </form>
        ...
        <section className="pt-10 sm:pt-14">
          <h2 className="mb-4 text-[19px] font-extrabold text-foreground">
            Popular services
          </h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {services.map(({ name, icon: Icon, tone }) => (
              <Link
                ...
                className="flex min-h-[74px] items-center justify-between rounded-[12px] border border-border bg-card px-4 text-foreground"
              >
                <span className="flex items-center gap-3">
                  <span
                    className={`flex h-9 w-9 items-center justify-center rounded-[9px] ${tone} text-primary`}
                  >
                    <Icon size={18} />
                  </span>
