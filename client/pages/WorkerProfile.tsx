...
import { ArrowRight, Check, MapPin, ShieldCheck, X, Heart } from "lucide-react";
...
export default function WorkerProfile() {
  ...
  return <PageShell backTo="/search" backLabel="Search results">
    <section className="rounded-[16px] border border-[#dcece7] bg-[#edf7f3] px-5 py-7 dark:border-white/10 dark:bg-[#151515] sm:px-8 sm:py-9">
      <div className="flex items-center gap-4"><span className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full text-lg font-extrabold text-navy bg-[#f5f6f4]">{worker.photo_url ? <img src={worker.photo_url} alt={worker.name} className="h-full w-full object-cover" /> : worker.initials}</span><div className="min-w-0"><h1 className="text-[27px] font-extrabold tracking-[-0.04em] text-navy dark:text-white">{worker.name}</h1><p className="mt-1 text-sm font-bold text-teal">{worker.category}</p><p className="mt-1 flex items-center gap-1.5 text-[13px] text-slate dark:text-slate-300"><MapPin size={14}/> {worker.locality}</p></div></div>
      <div className="mt-6 grid gap-2 sm:grid-cols-2"><button type="button" onClick={save} className="flex h-11 items-center justify-center gap-2 rounded-[10px] border border-[#b9ddd4] bg-white text-sm font-bold text-teal dark:border-[#555] dark:bg-[#222] dark:text-white"><Heart size={17} fill={saved ? "currentColor" : "none"}/>{saved ? "Saved" : "Save worker"}</button><button type="button" onClick={() => setShowContact(true)} aria-label={`Contact ${worker.name}`} className={`${softWhiteButton} flex h-11 items-center justify-center gap-2 rounded-[10px] text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal focus-visible:ring-offset-2`}>Contact <ArrowRight size={16}/></button></div>
    </section>
