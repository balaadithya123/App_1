...
const APP1_SUPABASE_PUBLISHABLE_KEY =
  "sb_publishable__ULoko5SRRH7WMIsO9ydkw_ZV18Nu3w";
...
class MockQueryBuilder {
  ...
  private execute() {
    ...
    if (this.isDelete) {
      ...
      return { data: [], error: null, count: rows.length - remaining.length };
    }
    ...
    auth: {
      async getUser(token: string) {
        if (!token)
          return {
            data: { user: null },
            error: { message: "Invalid session or token" },
          };
        return {
          data: {
            user: {
