...
describe("pure-JS word shingle duplicate text detection", () => {
  it("tokenizes and cleans text into lowercase words", () => {
    const tokens = tokenizeText(
      "Hello, World! Experienced Electrician in Chennai.",
    );
  ...
  it("detects near-duplicate profile descriptions using checkProfileTextSimilarity", () => {
    ...
    const newProfileText =
      "Experienced electrician specializing in home wiring, fuse box repair, solar installation, and ceiling fan repair in Cuddalore area.";

    const result = checkProfileTextSimilarity(
      newProfileText,
      existingProfiles,
      { threshold: 0.6 },
    );
  ...
  it("returns non-duplicate when profiles have low text similarity", () => {
    ...
    const newProfileText =
      "Licensed plumber fixing water pipe leakage, drain clearing, tap replacement, and bathroom fittings.";
...
describe("profile photo duplicate / stock check fallback", () => {
  it("flags stock photos correctly in fallback mode", async () => {
    const result = await checkProfilePhotoDuplicates(
      "https://example.com/stock-photo-electrician.jpg",
    );
  ...
  it("flags screenshot / meme photos correctly in fallback mode", async () => {
    const result = await checkProfilePhotoDuplicates(
      "https://example.com/wa_screenshot_profile.png",
    );
