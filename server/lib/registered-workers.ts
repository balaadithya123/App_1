...
const workerSelect = "id,name,phone,category,locality,experience,initials,tone,about,services,photo_url,created_at,available_today,away_from,away_until,urgent_today";
...
export const updateWorkerAvailabilityByPhone = async (phone: string, changes: { available_today: boolean; away_from?: string | null; away_until?: string | null; urgent_today: boolean }) => {
  const { data, error } = await supabase.from("workers").update(changes).eq("phone", phone).select(workerSelect).maybeSingle();
  if (error) throw new Error(`Unable to save worker availability: ${error.message}`);
