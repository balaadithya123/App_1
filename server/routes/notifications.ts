...
export const handleWatchWorker: RequestHandler = async (req, res) => {
  try {
    ...
    const { workerId } = z
      .object({ workerId: z.string().trim().min(1) })
      .parse(req.body);
    ...
    const alreadyAvailable =
      Boolean(worker.available_today) &&
      !(worker.away_until && worker.away_until >= today);
    if (alreadyAvailable)
      return res.json({ watching: false, alreadyAvailable: true });
    ...
    if (!email)
      return res
        ...
        .json({
          message:
            "Your account does not have an email address for notifications.",
        } satisfies ApiErrorResponse);
    ...
    return res
      ...
      .json({
        message:
          code === "UNAUTHORIZED"
            ? "Please log in to receive availability notifications."
            : code || "Unable to set notification.",
      } satisfies ApiErrorResponse);
  }
};
...
export const handleGetNotifications: RequestHandler = async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("notifications")
      .select("id,type,title,message,worker_id,read_at,created_at")
    ...
    return res
      .status(code === "UNAUTHORIZED" ? 401 : 500)
      .json({ message: code } satisfies ApiErrorResponse);
  }
};
