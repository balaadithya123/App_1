...
export const handleGetWorkerCallbackRequests: RequestHandler = async (
  req,
  res,
) => {
  try {
    ...
    for (const worker of allWorkers) {
      if (
        (phone && normalizePhone(worker.phone) === phone) ||
        (name && normalizeName(worker.name) === name)
      ) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id,client_name,client_phone,service_needed,preferred_time,notes,created_at,status",
      )
...
export const handleDeleteWorkerCallbackRequest: RequestHandler = async (
  req,
  res,
) => {
  try {
    ...
    return res.status(status).json({
      message:
        error instanceof Error
          ? error.message
          : "Unable to delete callback request.",
    });
  }
};

export const handleUpdateWorkerCallbackStatus: RequestHandler = async (
  req,
  res,
) => {
  try {
