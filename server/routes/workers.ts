...
import {
  ...
  saveRegisteredWorker,
  updateWorkerPhotoByPhone,
  updateWorkerProfileByPhone,
} from "../lib/registered-workers";
...
export const createWorker = (r: WorkerRegistration, existing: Worker[]): Worker => ({
  ...
  initials: createInitials(r.fullName),
  tone: "bg-[#f5f6f4]",
  about: r.about,
  services: r.services
    .split(",")
...
export const handleGetWorkers: RequestHandler = async (_req, res) => {
  try {
    ...
    const { data: verificationRows } = ids.length
...
export const handleRegisterWorker: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // 3. Pure-JS word-shingle text similarity check
      const newProfileText = `${result.data.fullName} ${result.data.category} ${result.data.location} ${result.data.about} ${result.data.services}`;
      const existingTextProfiles = existingWorkers.map((w) => ({
        id: w.id,
        name: w.name,
    ...
    return res.status(201).json({
      ...
      worker,
    } satisfies WorkerRegistrationSuccessResponse);
  } catch (error) {
  ...
  try {
    ...
    const body = z
      .object({
        ...
        away_from: z.string().date().nullable().optional(),
        away_until: z.string().date().nullable().optional(),
        urgent_today: z.boolean(),
      })
      .parse(req.body);
...
export const handleUpdateWorkerProfile: RequestHandler = async (req, res) => {
  try {
    ...
    const worker = await updateWorkerProfileByPhone(phone, body);

    // Silent background word-shingle text check and photo duplicate check
    try {
      const existingWorkers = await getAllWorkers();
      const otherWorkers = existingWorkers.filter((w) => w.id !== worker.id);

      const newProfileText = `${body.name} ${body.category} ${body.location} ${body.about} ${body.services.join(" ")}`;
      const existingTextProfiles = otherWorkers.map((w) => ({
        id: w.id,
        name: w.name,
        text: `${w.name} ${w.category} ${w.locality} ${w.about || ""} ${(w.services || []).join(" ")}`,
      }));
      ...
      if (body.photo_url) {
        const photoCheck = await checkProfilePhotoDuplicates(body.photo_url);
        if (photoCheck.is_stock_photo) {
          await addTrustFlag({
            worker_id: worker.id,
            flag_type: "stock_photo_detected",
      ...
      console.warn("[workers] background profile update check note:", checkErr);
    }
...
export const handleUpdateWorkerPhoto: RequestHandler = async (req, res) => {
  try {
    ...
    if (!photoUrl.success)
      return res.status(400).json({
        message: "A valid profile photo URL is required.",
      } satisfies ApiErrorResponse);
    ...
    try {
      const photoCheck = await checkProfilePhotoDuplicates(photoUrl.data);
      if (photoCheck.is_stock_photo) {
        await addTrustFlag({
          worker_id: worker.id,
          flag_type: "stock_photo_detected",
        ...
        await addTrustFlag({
          worker_id: worker.id,
          flag_type: "duplicate_photo_detected",
          reason: "Profile photo detected as duplicate or screenshot image",
        });
      } else {
