...
import {
  ...
  handleUpdateWorkerAvailability,
  handleUpdateWorkerPhoto,
...
import {
  handleGetWorkerCallbackRequests,
  handleDeleteWorkerCallbackRequest,
  handleUpdateWorkerCallbackStatus,
} from "./routes/callback-requests";
import {
  handleGetWorkerStats,
  handleRecordWorkerReferral,
} from "./routes/growth";
import {
  handleGetAgencies,
  handleGetAgencyTeam,
...
import {
  handleCreateTrackRecord,
  handleGetWorkerTrackRecord,
} from "./routes/track-record";
...
export function createServer() {
  ...
  app.get("/api/maps/geocode", handleGeocode);
  app.get("/api/maps/reverse-geocode", handleReverseGeocode);
  app.post("/api/chat", handleServiceChat);
  app.post("/api/voice-onboarding", handleVoiceOnboarding);
  app.post("/api/portfolio/screen", handleScreenPortfolio);
  app.post("/api/ranking/evaluate", handleEvaluateRanking);
  ...
  app.post("/api/phone-verification/complete", handleCompletePhoneVerification);
  app.get("/api/callback-requests", handleGetWorkerCallbackRequests);
  app.delete("/api/callback-requests/:id", handleDeleteWorkerCallbackRequest);
  ...
  app.get("/api/agencies/dashboard", handleGetAgencyDashboard);
  app.patch("/api/agencies/callbacks/:id", handleUpdateAgencyCallbackStatus);
  app.post("/api/agencies/regenerate-code", handleRegenerateAgencyCode);
  app.post("/api/agencies/join", handleJoinAgency);
  app.post("/api/agencies/leave", handleLeaveAgency);
  app.post("/api/agencies/remove-worker", handleRemoveWorkerFromAgency);
  app.get("/api/agencies/projects", handleGetAgencyProjects);
  app.post("/api/agencies/projects", handleCreateAgencyProject);
  app.delete("/api/agencies/projects/:id", handleDeleteAgencyProject);
  ...
  app.patch(
    "/api/workers/projects/:id/status",
    handleUpdateWorkerProjectStatus,
  );
  ...
  app.post(
    "/api/admin/workers/:workerId/resolve-flags",
    handleResolveWorkerFlags,
  );
  app.post(
    "/api/admin/workers/:workerId/toggle-verify",
    handleToggleWorkerVerification,
  );
  ...
  app.get("/api/download-zip", (_req, res) => {
    ...
    res.download(file, "project-source.zip", (err) => {
      if (err && !res.headersSent) {
        res
          .status(500)
          .json({ error: "Failed to download zip", details: String(err) });
      }
    });
  });
  return app;
